...
GH_REPO="${AGENT_GH_REPO:-AMVARA-CONSULTING/km0-mail}"
...
prepare_001_preflight_context() {
  ...
  {
    echo ""
    echo "=== Docker log incident heuristics (km0-mail stack) ==="
  } >>"$ctx"
  ...
  if docker info >/dev/null 2>&1; then
    local c raw hits
    for c in km0-mail-postfix-1 km0-mail-dovecot-1 km0-mail-rspamd-1 km0-mail-roundcube-1 km0-mail-postgres-1; do
...
(absolute: ${ctx})
Then follow 001-gh-reviewer.md — (A) GitHub → up to 3 × FEAT-*.md (issue_checker may have already created some). (B) Docker logs → NEW-*.md only for real standing incidents. gh comment/label for any FEAT you create manually. Task conventions: autoagents/TASKS-README.md. Do your job."
...
Environment:
  AGENT_GH_REPO              GitHub repo (default: AMVARA-CONSULTING/km0-mail)
