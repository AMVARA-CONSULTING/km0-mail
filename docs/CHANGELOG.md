...
- Roundcube webmail image `1.6.9-apache` → `1.6.18-apache` (1.6 LTS security current, 2026-08-09). Postgres, Dovecot, Postfix, and mail volumes were left running; only the roundcube container was recreated. Stay off 1.7.x until a dedicated `public_html`/`static.php` cutover (the 1.6.18 Apache image already uses `DocumentRoot /var/www/html/public_html` with skins/plugins symlinks, so km0 bind-mounts keep working).
...
- Single Proton-style login (password-only): `mail.km0digital.com/` now serves the native Roundcube login form directly (proxied, no redirect — a 302 broke the login POST since Roundcube posts the form to `/?_task=login`); the legacy `/login.html` `302`-redirects to `/index.php?_task=login` and the static branded landing is retired. OAuth is disabled in `config/roundcube/config.inc.php` (`oauth_provider = null`) so the "Login with KM0 Mail" button and the Dex redirect loop are gone (the `km0_sso_provision` plugin stays dormant — its `oauth_login` hook never fires). The skin login page (`skins/km0/templates/login.html` + `i18n.js`) drops the `auth.km0digital.com` "Other sign-in options"/hardcoded register links and the Google/activate banners; its only secondary link is a local "Create a free account" → `/register`, and a `registered=1` banner confirms signup. `/register` is simplified to username + password → `username@km0digital.com` (custom-domain selector + contact field removed; adding more addresses/domains is a post-login phase 2). Runbook URL table + auth-tracks updated
...
- Autoagents Redmine sync: stop posting `time_entry` / spent hours; keep *Time taken* in the closing note only (informational). Drop `REDMINE_ACTIVITY_ID`; omit duration when `Closed at (UTC)` is missing instead of inventing end times

### Fixed

- Autoagents Redmine duration label: skip *Time taken* when `Closed at (UTC)` is missing instead of falling back to `datetime.now()` (avoids inflated durations)

### Added

- Self-service custom domains (Phase 2): new `km0_domains` Roundcube plugin adds a **Settings → My domains** section where any authenticated user adds a domain, publishes DNS, verifies it, and creates/manages multiple addresses on it (`user@customdomain`). Backed by new owner-scoped, Bearer-protected `mail-provision-api` endpoints (`/my/domains`, `/my/addresses` with GET/POST/DELETE) and the existing `domain-verify-api` status/check; the plugin calls both server-to-server so no token reaches the browser. Ownership tracked by `mail_domains.owner_email` (`sql/init/06-domain-owner-email.sql`; `/register` custom now sets it). Operators use the same UI (an operator is just a user)
